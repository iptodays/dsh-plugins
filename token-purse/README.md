...
- 面板底部 **调整费率**：展开后是币种 / 汇率两行，加一张**每行一个 provider/model 的费率表**
  （直接改数字），原始 JSON 收在 **高级 JSON** 里；配置存在浏览器本地。
...
1. **推荐**：点开徽标 → **调整费率**，直接改费率表里的数字后保存；需要写 **peak**、
   **fx** 这类结构化表格没覆盖的字段时，展开 **高级 JSON**。配置存在浏览器
...
- **0.1.21**：修**费率表溢出**。0.1.20 把表格做成「1fr + 4 列 52px」，但输入框仍是
  76px 固定宽——比轨道宽 24px，四个数字框**互相重叠**并溢出面板（面板 overflow-x:hidden，
  右边被直接裁掉）；模型名所在的 1fr 轨道也缺 min-width:0，nowrap 让它的最小宽度等于
  整串文字（最长 37 字符约 222px），整行固有宽度约 442px 塞进 296px 的内容区。
  现在键独占一行、四列等宽流式，输入框 width:100% 跟随轨道；表头去掉重复的说明文字。
- **0.1.20**：按评审结论收敛**常驻文字**，并重做**费率编辑器**。
  - 覆盖口径说明从 42px 一段话降级成合计下方的 10px 脚注，并删掉与副标题重复的部分；
    「仅供参考」收进合计的 **title**。
  - 峰谷窗口与倍率搬进 **峰谷** 页签（触发器的悬浮提示里本来就有），不再常驻。
  - **调整费率** 不再只有一块 JSON：常用字段摊成**每行一个 provider/model 的表格**
    （输入 / 缓存读 / 缓存写 / 输出 四个数字框），原始 JSON 退到 **高级 JSON** 后面。
